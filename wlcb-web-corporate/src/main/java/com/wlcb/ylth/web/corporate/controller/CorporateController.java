package com.wlcb.ylth.web.corporate.controller;
...
//        }
    ...
    /**
     * @Author 郭丁志
     * @Description //TODO 企业法人列表
     * @Date 21:57 2020-03-02
     * @Param [corporateReview, request, response]
     * @return ResponseData
     **/
    @RequestMapping(value = "/listCorporate",method = {RequestMethod.GET,RequestMethod.POST},produces="application/json")
    public ResponseData listCorporate(TblCsrrgCorporate corporate, HttpServletRequest request, HttpServletResponse response){

        PageInfo<TblCsrrgCorporate> corporates = corporateService.listCorporate(corporate);

        return ReturnJsonUtil.printJson(200,"查询成功",corporates,true);
    }

    /**
     * @Author 郭丁志
     * @Description //TODO  更新企业法人信息
     * @Date 21:57 2020-03-02
     * @Param [corporateReview, request, response]
     * @return ResponseData
     **/
    @RequestMapping(value = "/updateCorporate",method = RequestMethod.PUT,produces="application/json")
    public ResponseData updateCorporate(TblCsrrgCorporate corporate, HttpServletRequest request, HttpServletResponse response){

        ResponseData responseData = BeanUtil.allFieldIsNULL(corporate,
                "id","updateUser");
        ...
        Integer count = corporateService.updateCorporate(corporate);

        if (count > 0){
            return ReturnJsonUtil.printJson(200,"更新成功",true);
        }else {
            return ReturnJsonUtil.printJson(Constants.RECODE_FAIL,"更新失败",false);
        }
