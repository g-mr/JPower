...
public class PostController extends BaseController {
	...
	public R<Long> add(@RequestBody @Validated(Validation.Create.class) CorePost corePost) {
		...
		if (Fc.isNull(corePost.getStatus())){
			corePost.setStatus(Boolean.TRUE);
		}
	...
	@Function(value = "启用停用",menus = {
			@Menu(client = "admin",menuCode = "POST",code = "POST_ENABLE",type = Menu.TYPE.BTN)
	})
	@Operation(summary = "启用停用")
	@PostMapping(value = "/enable/{id}", produces = "application/json")
	public R<Boolean> enable(@Parameter(description = "主键") @NotNull(message = "主键不可为空") @PathVariable("id") Long id,
							 @Parameter(description = "状态") @NotNull(message = "状态不可为空") @RequestSingleBody("status") Boolean status) {
		return R.status(postService.enable(id, status));
	}
